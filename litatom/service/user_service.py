...
class UserService(object):
    ...
    def search_user(cls, nickname, user_id):
        ...
        for obj in User.get_users_by_nickname(nickname):
            ...
            if cnt >= max_num:
                break
        if len(nickname) >= 3 and cnt <= max_num:
    ...
    def _on_create_new_user(cls, user):
        ...
        if UserSetting.user_num_by_uuid(request.uuid) >= cls.UUID_MAX_REGISTER_NUM - 1 and not GlobalizationService.help_escape_develop():
