...
class DiamStatService(object):
    ...
    def diam_stat_report(cls, date=datetime.datetime.now()):
        ...
        for loc in data:
            if loc == 'ALL':
                data[loc].append(User.objects(vip_time__gte=date_to_int_time(next_date(date,30*ONE_DAY)),
                                              vip_time__lte=date_to_int_time(next_date(date,31*ONE_DAY))).count())
            else:
                data[loc].append(User.objects(vip_time__gte=date_to_int_time(next_date(date,30*ONE_DAY)),
                                              vip_time__lte=date_to_int_time(next_date(date,31*ONE_DAY)),
                                              country=loc).count())
