...
class UserRecord(Document):
    ...
    def get_forbid_users_by_time(cls, from_ts, to_ts, forbid_type=None):
        if not forbid_type:
            return cls.objects(create_time__gte=from_ts, create_time__lte=to_ts).order_by('create_time')
        return cls.objects(create_time__gte=from_ts, create_time__lte=to_ts, action=forbid_type).order_by('create_time')
